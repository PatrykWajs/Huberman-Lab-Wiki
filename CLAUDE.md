...
A static MkDocs website containing summaries + full transcripts for 409 Huberman Lab episodes (as of 2026-06-02), deployed to GitHub Pages. Built and maintained by AI. Not affiliated with Huberman Lab officially.
...
| 2026-06-02 | 409 | `_DfqnpSbMfE` | Peptides: The Science, Uses & Safety | Dr. Abud Bakri, Internal Medicine Physician | 5,522 | supplementation *(manually patched — `parse_frontmatter` first-topic-drop bug)*, hormone-health, aging-and-longevity-science, general-health | pending |
...
- **2026-06-02 — parse_frontmatter() first-topic-drop bug FIXED (historical: this has silently been losing 1 Conclusion write per episode since the function was written).** EP-409 (*Peptides: The Science, Uses & Safety*, Dr. Abud Bakri, `_DfqnpSbMfE`, 5,522 segments) processed cleanly via Tier 1 (now 8-for-8 streak from 2026-05-12 → 2026-06-02), but the pipeline log printed `Topics: ['Hormone Health', 'Aging and Longevity Science', 'General Health']` — only 3 of the 4 declared in `summary.md` frontmatter (Supplementation was item 1 and was missing). Investigating revealed `parse_frontmatter()` had an inverted-condition bug: after seeing `topics:` it set a `_last = 'topics'` flag, then guarded the `- foo` append with `'topics' not in result.get('_last', '')` — which evaluates to False right after `_last` is set to `'topics'`, so the FIRST list item is always sent to the else branch (which then resets `_last = ''`), making subsequent items work but losing item 1. Net effect: every episode in the wiki's entire history has been missing one Conclusion write (whichever topic Haiku placed first in the frontmatter). Fixed in `pipeline.py` by rewriting `parse_frontmatter()` to use an explicit `in_topics` boolean that toggles ON when a `topics:` line is seen and OFF when the next non-list-item line appears — every list item is now captured, verified with the EP-409 frontmatter as a test fixture (returned all 4 topics). Hand-patched the missed EP-409 Supplementation finding into `docs/Conclusions/supplementation.md` (3 entries from the summary's first 3 Key Findings). Also caught a Haiku guest-name typo on EP-409 — `Dr. Abu Bakri` (missing a `d`) vs the YouTube canonical `Dr. Abud Bakri` — corrected in both `summary.md` frontmatter and `docs/GUESTS.md` heading. **Historical backfill (not done):** every existing episode is technically missing 1 Conclusion write. Backfill would touch 100s of files; deferred indefinitely — the bug fix prevents future loss, which is the main thing.

- **2026-05-30 — TOPIC_ALIASES fix deployed for Haiku slug-splitting bug.** EP-408 (*Essentials: Healing from Grief*, solo, `PlQyGIIAGNo`, 894 segments) processed cleanly via Tier 1 (now 7-for-7 streak from 2026-05-12 → 2026-05-30), but exposed a previously-unobserved class of pipeline failure: Haiku emitted `'Meditation and Breathwork'` and `'NSDR'` as TWO separate strings in the summary's `topics:` frontmatter instead of the single canonical `"NSDR, Meditation and Breathwork"`. Both got `TOPIC_MAP.get(...) → None`, both triggered `WARN: Unknown topic '...' — skipping conclusions update`, and `nsdr-meditation-and-breathwork.md` received zero automated findings. Caught the warnings in stdout before push and hand-appended 2 findings from the summary's NSDR-tagged Key Findings to the file (commit `28f950f` for the EP-408 batch). Then deployed a durable fix in commit `5f15cc7` — added a new `TOPIC_ALIASES` dict at module top of `pipeline.py` covering every split form of both comma-joined `TOPIC_MAP` keys (`"NSDR, Meditation and Breathwork"` and `"Alcohol, Tobacco and Cannabis"`), and rewrote `update_conclusions()` to consult `TOPIC_ALIASES` before `TOPIC_MAP`, emit `ALIAS: 'X' → 'Y'` log lines on recovery, and dedup via a `written` set so two split-form variants targeting the same file don't double-write findings. Verified the dispatch on 8 test cases (split forms + canonical forms + unknowns) — all map to the expected files. **Forward rule:** any future expansion of `TOPIC_MAP` with a comma-joined key requires a matching `TOPIC_ALIASES` block for each split variant. Still grep stdout for `WARN: Unknown topic` after each run as a safety net for unanticipated splits.
